...
def test_add_tag_to_set():
    ...
    rock_case_set = set()
    add_tag_to_set("ROCK", tags, rock_case_set)
    assert rock_case_set == {"rock"}

    tags = ["rock", "classical", "hip hop", "hip-hop"]
    hiphop_case_set = set()
    add_tag_to_set("hip hop", tags, hiphop_case_set)
    assert hiphop_case_set == {"hip hop", "hip-hop"}

    tags = ["rock", "classical", "hip hop", "hip-hop"]
    hiphop_direct_case_set = set()
    add_tag_to_set("hip-hop", tags, hiphop_direct_case_set)
    assert hiphop_direct_case_set == {"hip hop", "hip-hop"}

    tags = ["rock", "classical", "hip hop", "hip-hop"]
    classical_case_set = set()
    add_tag_to_set("classical", tags, classical_case_set)
    assert classical_case_set == {"classical"}
...
def test_tags_match():
    assert tags_match(TRACKS[0], {"psychedelic rock", "classical"})

    assert not tags_match(TRACKS[0], {"classical", "pop"})
...
def test_create_playlist():
    ...
    playlist = create_playlist(tracks, {"wonky", "synthpop"}, 7)
    assert playlist == [tracks[1], tracks[2]]

    playlist = create_playlist(tracks, {"wonky", "synthpop", "psychedelic rock"}, 10)
    assert playlist == [tracks[0], tracks[1], tracks[2]]
